...
contains
  ...
  subroutine allocate_mesh(mesh, nodes, elements, shape, name)
    ...
    if(name .eq. "InputMeshCoordinate")then
    allocate(mesh%ndglno(elements*shape%loc))
    else
    CALL offload_create_buffer_c(length=4*elements*shape%loc, buffer=mesh%pinned_ndglno)
    CALL C_F_POINTER(mesh%pinned_ndglno,mesh%ndglno, shape=(/elements*shape%loc/))
    mesh%pinned_alloc=.true. 
    end if
#ifdef _OPENMP
  ...
  subroutine allocate_scalar_field(field, mesh, name, field_type, py_func, py_positions)
    ...
    CALL offload_create_buffer_c(length=8*node_count(mesh), buffer=field%pinned_val)
    CALL C_F_POINTER(field%pinned_val,field%val, shape=(/node_count(mesh)/))
    field%pinned_alloc=.true.
  ...
  subroutine allocate_vector_field(field, dim, mesh, name, field_type)
   ...
   !   allocate(field%val(dim,n_count))
    CALL offload_create_buffer_c(length=8*dim*n_count, buffer=field%pinned_val)
    CALL C_F_POINTER(field%pinned_val,field%val, shape=(/dim,n_count/))
    field%pinned_alloc=.true.
#ifdef HAVE_MEMORY_STATS
  ...
  subroutine allocate_tensor_field(field, mesh, name, field_type, dim)
    ...
    CALL offload_create_buffer_c(length=8*field%dim(1)*field%dim(2)*node_count(mesh), buffer=field%pinned_val)
    CALL C_F_POINTER(field%pinned_val,field%val, shape=(/field%dim(1),field%dim(2),node_count(mesh)/))
    field%pinned_alloc=.true.
#ifdef HAVE_MEMORY_STATS
...
#endif
       if(mesh%pinned_alloc) then
         CALL offload_free_buffer_c(mesh%pinned_ndglno)
         mesh%pinned_alloc=.false.
         nullify(mesh%ndglno)
       else 
...
#endif
       if(field%pinned_alloc)then
         CALL offload_free_buffer_c(field%pinned_val)
         field%pinned_alloc=.false.
         nullify(field%val)
       else
