...
module transform_elements
  ...
  public :: transform_to_physical, transform_facet_to_physical, &
            ...
            retrieve_cached_valid_for_GPU,transform_to_physical_full_GPU
  
  integer, parameter :: cyc3(1:5)=(/ 1, 2, 3, 1, 2 /)  

  logical, save :: cache_transform_elements=.true.
  logical, save :: cache_cuda_allocated=.false.
...
!   real, pointer ::invJ_cache(:,:,:)
!   real, pointer ::J_T_cache(:,:,:)
!   real, pointer ::detJ_cache(:)
!   save ::invJ_cache
!   save ::J_T_cache
!   save ::detJ_cache
! used for un-cached transform-physical and get-edge-lengths
   INTEGER(C_INTPTR_T), save :: ptr_invJ_cache
   INTEGER(C_INTPTR_T), save :: ptr_detJ_cache
...
!    ewrite(1,*) "Reconstructing element geometry cache."
    ...
    if (allocated(invJ_cache) .or. cache_cuda_allocated) then
    !if (cache_cuda_allocated) then
#ifdef HAVE_MEMORY_STATS
       call register_deallocation("transform_cache", &
            "real", size(invJ_cache)+size(J_T_cache)+size(detJ_cache))
#endif
      !call deallocate_cuda_pinned_mem(invJ_cache, J_T_cache, detJ_cache)
      deallocate(invJ_cache, J_T_cache, detJ_cache)
    end if

    elements=element_count(X)

   !  call allocate_cuda_pinned_mem(invJ_cache, J_T_cache, detJ_cache, X%dim, elements)
   !  cache_cuda_allocated = .true.
    ...
    !记得把三个数组结果拷贝回cpu这，但是要留2个数组再GPU
    
   !  call cpu_time(start_time)

   !  call compute_jacobi_gpu(X%val, x_shape%dn, X%mesh%ndglno,&
   !  size(X%val,2),X%dim, x_shape%ngi, x_shape%loc,elements,&
   !  ptr_invJ_cache,ptr_detJ_cache,&
   !  J_T_cache, invJ_cache, detJ_cache)

   !  call cpu_time(end_time)
   !  elapsed_time = end_time - start_time
   !  write(*,*) 'GPU mode construct_Jacobi_cache Elapsed time: ', elapsed_time, ' seconds' 
   ...
   !  call cpu_time(start_time)
    ...
    call cpu_time(end_time)
    elapsed_time = end_time - start_time
    write(*,*) 'CPU mode construct_Jacobi_cache Elapsed time: ', elapsed_time, ' seconds' 
   !  stop
...
function retrieve_cached_valid_for_GPU(X) result (cache_valid)
    !!< Determine whether the transform cache is valid for this operation.
    !!<
    !!< If caching is applicable and the cache is not ready, set up the
    !!< cache and then return true.
    ...
    cache_valid = is_cache_valid(X) !true
    !以下不执行,因为这是按ele获得value，但是我们将全部的value copy到global mem，在kernel中分thread获取
    !if (.not. cache_valid) return   
    ! !in assemble kmk case
    ! !*_cache变量都具有save属性，在下次调用函数时，依然保留上一次的值
    ! J_local_T=J_T_cache(:, :, ele) !J_T_cache存了所有ele的3*3 Jacobi矩阵
    ! invJ_local=invJ_cache(:, :, ele)
    ! detJ_local=detJ_cache(ele) !jacobi的行列式值

end function retrieve_cached_valid_for_GPU


subroutine transform_to_physical_full_GPU(gpu_on, X, shape, ele_count, &
   &x_shape)
   !!<GPU mode of transform_to_physical_full, do conditions judgement in CPU then call GPU kernel>
   !! should modify the parameters:J, invJ, detJ
   logical, intent(in) :: gpu_on
    !! X is the positions field.
   !! X%val contains x,y,z ordinates of all nodes
    type(vector_field), intent(in) :: X
    ...
    type(element_type), intent(in), optional, target :: X_shape
     ...
     !!*********************************we removed J, invJ, detJ
   !!*********************************we add
   !! This is the num of iteration
   integer :: ele_count
   !! This is the address of dshape on GPU, so that dshape_tensor_dshape can access to this information externally
   !! GPU上的地址信息，dshape、detwei的结果存储在global memory中，dshape_tensor_dshape之后还要计算
   !! 确定下这样可以成功传递地址信息吗？还是要用pointer类型，
   !! 有疑虑 因为fortran的ptr是个大小指定的数组
   !!
   logical :: detwei_exist
...
!       cache_valid=retrieve_cached_valid_for_GPU(X)
!       if (cache_valid) then
!         write(*,*) "will call transform_to_physical_full_GPU_cached_C with cached in global mem"
!         call transform_to_physical_full_GPU_cached_C(m_nonlinear,shape%dn,lx_shape%quadrature%weight,&
!         lx_shape%ngi,X%dim,shape%loc,ele_count,invJ_cache,detJ_cache,h_dshape,h_detwei,d_dshape,d_detwei,detwei_exist,&
!         X%val, X%mesh%ndglno, d_field_val, d_ndglno, size(X%val,2), ptr_invJ_cache, ptr_detJ_cache)
!         return
!     end if
!   else
   cache_valid = .false.
!   end if
!
  !调用非cached的GPU kernel
  
  ndglno_size = ele_count*4
  write(*,*) "will call transform_to_physical_full_GPU_C without cached invJ"
   call transform_to_physical_full_GPU_C(X%val, shape%dn, lx_shape%dn, &
   lx_shape%quadrature%weight,X%mesh%ndglno,size(X%val,2),ndglno_size,&
   x_spherical,x_nonlinear,m_nonlinear,&
   X%dim,lx_shape%ngi,shape%loc,ele_count,&
   detwei_exist)
   !lx_shape%dn and X_val are used to compute J_local_T

  end subroutine transform_to_physical_full_GPU
