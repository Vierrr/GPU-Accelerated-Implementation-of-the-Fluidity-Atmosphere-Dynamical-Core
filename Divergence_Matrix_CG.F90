...
module divergence_matrix_cg
  ...
  contains
    ...
    subroutine assemble_1mat_compressible_divergence_matrix_cg(ctp_m, state, istate, ct_rhs, div_mass) 
      ...
      !real, dimension(:,:,:,:), allocatable :: loop_ele_mat; 
      ...
      !allocate(loop_ele_mat(3,4,4,element_count(test_mesh))) 
      start_time=MPI_Wtime()
      call div_create_stream()
      call div_alloc_gpu_mem(element_count(test_mesh),test_mesh%nodes)
      call div_shape_dn_to_device(coordinate%mesh%shape%dn,coordinate%mesh%shape%dn,&
           coordinate%mesh%shape%quadrature%weight) 
      call div_set_gpu_data(element_count(test_mesh),test_mesh%nodes,&
      test_mesh%ndglno,test_mesh%shape%n,density%val,&
      olddensity%val,pressure%val,coordinate%val,field%val)
      call gpu_div_mat_assemble(element_count(test_mesh),test_mesh%nodes,theta)
!      do ele=1, element_count(test_mesh)
...
!                                   (1-theta)*(ele_grad_at_quad(olddensity, ele, ddensity_t))
        !loop_ele_mat(:,:,:,ele) = shape_dshape(test_shape, dfield_t, &
...
!      enddo
   ...
   call alloc_adv_assemble(ctp_m%sparsity%findrm, ctp_m%sparsity%colm, pressure%mesh%ndglno, &
   ele_loc(pressure, 1), ele_ngi(pressure, 1), element_count(pressure%mesh), &
   size(ctp_m%sparsity%findrm,1),size(ctp_m%val(1,1)%ptr,1))
     
   
   do dim = 1, field%dim
   call div_assemble_csr_matrix_gpu(ctp_m%val(1,dim)%ptr,&
   ele_loc(pressure, 1), ele_ngi(pressure, 1), element_count(pressure%mesh), &
   size(ctp_m%sparsity%findrm,1),size(ctp_m%val(1,dim)%ptr,1),dim)
   end do
   end_time=MPI_Wtime()
   elapsed_time=end_time - start_time
   write(*,*)"Assemble the divergence matrix:",elapsed_time
   !do ele=1, element_count(test_mesh)
   !     test_nodes=>ele_nodes(test_mesh, ele)
   !     field_nodes=>ele_nodes(field, ele)
   !     do dim = 1, field%dim
            !here write(*,*)"addto"
      !      call addto(ctp_m, 1, dim, test_nodes, field_nodes, loop_ele_mat(dim,:,:,ele))
   !     end do
   !   enddo

      call dealloc_adv_assemble()
      call dealloc_div_mem()
      !deallocate(loop_ele_mat)
      call div_destroy_stream()
      if(multiphase) then
         deallocate(dnvfrac_t)
         call deallocate(nvfrac)
      end if
    write(*,*)"Exit assemble_1mat_compressible_divergence_matrix_cg"  
