...
!       +source)
  ...
  subroutine assemble_1mat_compressible_projection_cg(state, cmc, rhs, dt, &
    ...
    if(cmcget) then
      ...
      !call compressible_eos(state, pressure=eospressure, drhodp=drhodp) ! FA Original Code 
      !2021-08-10-005S======== Atmospheric Option for Atmospheric State Equation   !@#$% jxli Aug 2021
      call compressible_eos(state, density=density, drhodp=drhodp) ! *Note*
      !2021-08-10-005E============================================================ !@#$% jxli Aug 2021
      ewrite_minmax(density)
      ewrite_minmax(olddensity)
      ...
      allocate(dtest_t(ele_loc(test_mesh, 1), ele_ngi(test_mesh, 1), field%dim), &
       ...
       call prj_create_stream()
       call prj_alloc_gpu_mem(element_count(test_mesh),test_mesh%nodes)
       call prj_shape_dn_to_device(coordinate%mesh%shape%dn,coordinate%mesh%shape%dn,&
           coordinate%mesh%shape%quadrature%weight)
       call prj_set_gpu_data(element_count(test_mesh),test_mesh%nodes,&
            test_mesh%ndglno,test_mesh%shape%n,density%val,&
            olddensity%val,pressure%val,drhodp%val,eospressure%val,rhs%val,coordinate%val)
       call gpu_prj_mat_assemble(element_count(test_mesh),test_mesh%nodes,&
            atmospheric_pressure,dt,theta_divergence,theta_pg,rhs%val)
       
   call alloc_adv_assemble(cmc%sparsity%findrm, cmc%sparsity%colm, pressure%mesh%ndglno, &
   ele_loc(pressure, 1), ele_ngi(pressure, 1), element_count(pressure%mesh), &
   size(cmc%sparsity%findrm,1),size(cmc%val,1))
   call assemble_csr_matrix_gpu(cmc%val,&
   ele_loc(pressure, 1), ele_ngi(pressure, 1), element_count(pressure%mesh), size(cmc%sparsity%findrm,1),size(cmc%val,1))

      end_time=MPI_Wtime()
      elapsed_time=end_time - start_time
     write(*,*) "Assemble the projection matrix:",elapsed_time
      call dealloc_adv_assemble()
       call dealloc_prj_mem()       
       call prj_destroy_stream()
