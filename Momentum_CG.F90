...
#include "fdebug.h"
  ...
  contains
    ...
    subroutine construct_momentum_cg(u, p, density, x, &
                                     ...
                                     !state, assemble_ct_matrix_here, include_pressure_and_continuity_bcs) ! FA Original Code
                                     state, assemble_ct_matrix_here, include_pressure_and_continuity_bcs, counterG_V) !@#$% jxli Apr 2023
      ...
      type(tensor_field), intent(out)     :: counterG_V
    ...
    integer :: assemble_ct_int
    real elapsed_time;
    real, dimension(:,:,:), allocatable ::  loop_big_m_diag_addto,loop_rhs_addto 
    real, dimension(:,:,:,:,:), allocatable ::loop_big_m_tensor_addto
    type(element_type), dimension(:),allocatable :: loop_test_function
    real, dimension(:,:), allocatable ::loop_detwei 
    real, dimension(:,:,:,:), allocatable ::loop_du_t
    real, dimension(:,:,:,:), allocatable ::loop_grad_p_u_mat
   
   allocate(loop_big_m_diag_addto(u%dim,4,element_count(u%mesh))) 
   allocate(loop_rhs_addto(u%dim,4,element_count(u%mesh))) 
   allocate(loop_big_m_tensor_addto(u%dim,3,4,4,element_count(u%mesh))) 
   allocate(loop_test_function(element_count(u%mesh)))
   allocate(loop_detwei(11, element_count(u%mesh)))
   allocate(loop_du_t(4,11,3,element_count(u%mesh)))
   allocate(loop_grad_p_u_mat(3,4,4,element_count(u%mesh))) 

   call create_streams()
   call alloc_gpu_momentum(element_count(u%mesh),u%mesh%nodes)
   call alloc_momentum_pinned_mem(element_count(u%mesh),u%mesh%nodes)
!   call set_gpu_data(element_count(u%mesh),u%mesh%nodes,&
!        u%mesh%ndglno,u%mesh%shape%n, viscosity%val,x%val,&
!        inverse_masslump%val,u%val,oldu%val,nu%val,source%val,&
!        ct_rhs%val,density%val)
      ...
      !20241014 lls
      have_mrf=have_option('/material_phase[0]/subgridscale_parameterisations/MRF/')
      !20241014 lls
   ...
   call shape_dn_to_device(u%mesh%shape%dn,x%mesh%shape%dn,x%mesh%shape%quadrature%weight)
   call set_gpu_data(element_count(u%mesh),u%mesh%nodes,&
        u%mesh%ndglno,u%mesh%shape%n, viscosity%val,x%val,&
        inverse_masslump%val,u%val,oldu%val,nu%val,rhs%val, source%val,&
        ct_rhs%val,density%val)
      
     assemble_ct_int = assemble_ct_matrix_here 
      call gpu_transform_to_physical_face(element_count(X%mesh),u%mesh%shape%dn, &
      & x%mesh%shape%dn,x%mesh%shape%quadrature%weight,loop_grad_p_u_mat(1,1,1,1))
     call gpu_viscosity_face(element_count(u%mesh),beta,dt, &
     &theta,smagorinsky_coefficient,loop_big_m_tensor_addto,assemble_ct_int)
    end_time = MPI_Wtime()
    elapsed_time = end_time - start_time
    write(*,*)"GPU assemble momentum:",elapsed_time
     ...
     start_time=MPI_Wtime()
     call pinned_copy_back(ele_count(u%mesh),u%mesh%nodes,loop_big_m_tensor_addto(1,1,1,1,1),&
        & inverse_masslump%val,loop_grad_p_u_mat(1,1,1,1))
      do ele = 1, element_count(u%mesh)
      u_ele=>ele_nodes(u, ele)
      call addto(big_m, u_ele, u_ele, loop_big_m_tensor_addto(:,:,:,:,ele), block_mask=block_mask)
      end do
    end_time = MPI_Wtime()
    elapsed_time = end_time - start_time
    start_time = MPI_Wtime()
    if(assemble_ct_matrix_here) then
      do ele = 1, element_count(u%mesh)
      ! gpuhehe write(*,*)"here assemble_ct_matrix_here"
      u_ele=>ele_nodes(u, ele)
      p_ele=>ele_nodes(ct_rhs, ele)
       call addto(ct_m, p_ele, u_ele, spread(loop_grad_p_u_mat(:,:,:,ele), 1, 1))
      end do
     endif
    ...
    elapsed_time = end_time - start_time
    !lls gpu 
    call dealloc_gpu_momentum()
    call dealloc_momentum_pinned_mem()
      ...
      contains 
      subroutine loop_add_diagonal_to_tensor(big_m_diag_addto, big_m_tensor_addto)
      ...
      end subroutine loop_add_diagonal_to_tensor
    ...
    subroutine construct_momentum_surface_element_cg(sele, big_m, rhs, ct_m, ct_rhs, &
      ...
      if (any(velocity_bc_type(:,sele)==BC_TYPE_FLUX)) then
        ...
        subroutine construct_momentum_element_cg_loop_1(state, ele, big_m, rhs, ct_m, &
                                            mass, masslump, &
                                            ...
                                            !alpha_u_field, abs_wd, temperature, nvfrac, supg_shape) ! FA Original Code  
                                  alpha_u_field, abs_wd, temperature, nvfrac,&
                          big_m_diag_addto,rhs_addto,big_m_tensor_addto,&
                         test_function,detwei,du_t,grad_p_u_mat,&
 supg_shape, counterG_V) !@#$% jxli Apr 2023 
      ...
      type(element_type), pointer :: u_shape, p_shape
      real, dimension(ele_ngi(u, ele)) ,intent(inout):: detwei
      real, dimension(ele_ngi(u, ele)) ::  detwei_old, detwei_new
      real, dimension(u%dim, u%dim, ele_ngi(u,ele)) :: J_mat, diff_q
      real, dimension(ele_loc(u, ele), ele_ngi(u, ele), u%dim),intent(inout) :: du_t
      ...
      real, dimension(u%dim, ele_loc(ct_rhs, ele), ele_loc(u, ele)),intent(inout) :: grad_p_u_mat
      
      ! What we will be adding to the matrix and RHS - assemble these as we
      ! go, so that we only do the calculations we really need
      real, dimension(u%dim, ele_loc(u, ele)),intent(inout) :: big_m_diag_addto, rhs_addto
      real, dimension(u%dim, u%dim, ele_loc(u, ele), ele_loc(u, ele)),intent(inout) :: big_m_tensor_addto
      logical, dimension(u%dim, u%dim) :: block_mask ! control whether the off diagonal entries are used
      integer :: dim, i, j
     ...
     !write(*,*)"ct_rhs loc", ele_loc(ct_rhs, ele), "ct_rhs ngi",ele_ngi(ct_rhs, ele) 
     !big_m_diag_addto = 0.0
     ! big_m_tensor_addto = 0.0
     ! rhs_addto = 0.0
      call construct_momentum_element_cg_part_1(state, ele, big_m, rhs, ct_m, &
                                            mass, masslump, &
                                            x, x_old, x_new, u, oldu, nu, ug, &
                                            density, ct_rhs, &
                                            source, absorption, buoyancy, hb_density, gravity, &
                                            ...
                                            assemble_ct_matrix_here, depth, &
                                            !alpha_u_field, abs_wd, temperature, nvfrac, supg_shape) ! FA Original Code  
                                            alpha_u_field, abs_wd, temperature, nvfrac, supg_shape, counterG_V,&
                                            big_m_diag_addto,big_m_tensor_addto,rhs_addto,test_function,detwei,detwei_old,detwei_new,du_t,grad_p_u_mat)
    end subroutine construct_momentum_element_cg_loop_1

        subroutine construct_momentum_element_cg_loop_2(state, ele, big_m, rhs, ct_m, &
                                            mass, masslump, &
                                            x, x_old, x_new, u, oldu, nu, ug, &
                                            density, ct_rhs, &
                                            source, absorption, buoyancy, hb_density, gravity, &
 ...
 supg_shape, counterG_V) !@#$% jxli Apr 2023 
     ...
     ! rhs_addto = 0.0
      ...
      !call gpu_ele_val_vector_oldu(ele,oldu_val) 
      ...
      if(have_viscosity .or. have_les) then
        call add_viscosity_element_cg_ff(state, ele, test_function, u, oldu_val, nu, x, viscosity, grad_u, &
           fnu, tnu, leonard, strainprod, alpha, gamma, du_t, detwei, big_m_tensor_addto, rhs_addto, temperature, density, nvfrac)
      end if
!           call construct_momentum_element_cg_part_2(state, ele, big_m, rhs, ct_m, &
!                                            mass, masslump, &
!                                            x, x_old, x_new, u, oldu, nu, ug, &
!                                            density, ct_rhs, &
!                                            source, absorption, buoyancy, hb_density, gravity, &
...
!                                            alpha_u_field, abs_wd, temperature, nvfrac, supg_shape, counterG_V,&
!                                            big_m_diag_addto,big_m_tensor_addto,rhs_addto,test_function,detwei,detwei_old,detwei_new,du_t,grad_p_u_mat)
    end subroutine construct_momentum_element_cg_loop_2
    
    
        subroutine construct_momentum_element_cg_loop_3(state, ele, big_m, rhs, ct_m, &
                                            mass, masslump, &
                                            x, x_old, x_new, u, oldu, nu, ug, &
                                            density, ct_rhs, &
                                            source, absorption, buoyancy, hb_density, gravity, &
 ...
 supg_shape, counterG_V) !@#$% jxli Apr 2023 
     ...
     ! big_m_tensor_addto = 0.0
     ! rhs_addto = 0.0
           call construct_momentum_element_cg_part_3(state, ele, big_m, rhs, ct_m, &
                                            mass, masslump, &
                                            x, x_old, x_new, u, oldu, nu, ug, &
                                            density, ct_rhs, &
                                            source, absorption, buoyancy, hb_density, gravity, &
                                            ...
                                            alpha_u_field, abs_wd, temperature, nvfrac, supg_shape, counterG_V,&
                                            big_m_diag_addto,big_m_tensor_addto,rhs_addto,test_function,detwei,detwei_old,detwei_new,du_t,grad_p_u_mat)
    end subroutine construct_momentum_element_cg_loop_3
    
    
    
    subroutine construct_momentum_element_cg_part_1(state, ele, big_m, rhs, ct_m, &
                                            mass, masslump, &
                                            x, x_old, x_new, u, oldu, nu, ug, &
                                            density, ct_rhs, &
                                            source, absorption, buoyancy, hb_density, gravity, &
                                            ...
                                            !alpha_u_field, abs_wd, temperature, nvfrac, supg_shape) ! FA Original Code  
                                            alpha_u_field, abs_wd, temperature, nvfrac, supg_shape, counterG_V,&
                                            big_m_diag_addto,big_m_tensor_addto,rhs_addto,test_function,detwei,detwei_old,detwei_new,du_t,grad_p_u_mat) !@#$% jxli Apr 2023 
      ...
      ! write(*,*)"here transform_to_physical"
      !  call transform_to_physical_debug(X, ele, &
      !                            u_shape, dshape=du_t, detwei=detwei)
      !  J_mat = 0.0
      else
        call transform_to_physical(x, ele, &
                                  u_shape, dshape=du_t, detwei=detwei, J=J_mat)
      ! write(*,*)"eles here transform_to_physical"
      end if
      !call exit(0)
      ...
      !here       write(*,*)"grad_p_u_mat,shape_dshape"
...
end subroutine construct_momentum_element_cg_part_1

subroutine construct_momentum_element_cg_part_2(state, ele, big_m, rhs, ct_m, &
                                            mass, masslump, &
                                            x, x_old, x_new, u, oldu, nu, ug, &
                                            density, ct_rhs, &
                                            source, absorption, buoyancy, hb_density, gravity, &
                                            ...
                                            test_function,detwei,detwei_old,detwei_new,du_t,grad_p_u_mat) !@#$% jxli Apr 2023 
    ...
    end subroutine construct_momentum_element_cg_part_2

subroutine construct_momentum_element_cg_part_3(state, ele, big_m, rhs, ct_m, &
                                            mass, masslump, &
                                            x, x_old, x_new, u, oldu, nu, ug, &
                                            density, ct_rhs, &
                                            source, absorption, buoyancy, hb_density, gravity, &
      ...
      if (have_mrf) then
         call add_MRF_countergradient_correction_momentum_element_cg(ele, u, du_t, detwei, rhs_addto, counterG_V)
      end if
    ...
    end subroutine construct_momentum_element_cg_part_3
    ...
    subroutine add_mass_element_cg(ele, test_function, u, oldu_val, density, nvfrac, detwei, detwei_old, detwei_new, big_m_diag_addto, big_m_tensor_addto, rhs_addto, mass, masslump)
      ...
      !here   write(*,*)"else move mesh mass_mat"
      ...
      if(assemble_inverse_masslump .and. compute_lumped_mass_here) then
        ...
        !here  write(*,*)"assemble inverse_masslump"
        do dim = 1, u%dim
    ...
    subroutine add_advection_element_cg(ele, test_function, u, oldu_val, nu, ug,  density, viscosity, nvfrac, du_t, dug_t, dnvfrac_t, detwei, J_mat, big_m_tensor_addto, rhs_addto)
      ...
      if(integrate_advection_by_parts) then
        ...
        !heregpu write(*,*)"else integrate_advection_by_parts "
      ...
      case(STABILISATION_STREAMLINE_UPWIND)
         if(have_viscosity) then
            !write(*,*)"STABILISATION_STREAMLINE_UPWIND"
            ...
            !write(*,*)"else have_viscosity STABILISATION_STREAMLINE_UPWIND"
            advection_mat = advection_mat + &
                 & element_upwind_stabilisation(u_shape, du_t, relu_gi, J_mat, detwei, &
    ...
    subroutine add_sources_element_cg(ele, test_function, u, density, source, detwei, rhs_addto)
      ...
      if(ele==100)then
       write(*,*)"density_gi"
       write(*,*)density_gi
       write(*,*)"density_gi*detwei"
       write(*,*)detwei*density_gi
      end if
      ...
      if(ele==100)then
       write(*,*)"fortran source_mat"
       write(*,*)source_mat
       write(*,*)"end fortran source_mat"
      end if
      if(lump_source) then
        !write(*,*) "lump_source"
      ...
      !heregpu  write(*,*) "else lump_source"
        do dim = 1, u%dim
          rhs_addto(dim, :) = rhs_addto(dim, :) + matmul(source_mat, ele_val(source, dim, ele))
      if(ele==100)then
      write(*,*)"field_type",source%field_type
       write(*,*)"source"
       write(*,*)ele_val(source, dim, ele)
       write(*,*)"matmul"
       write(*,*) matmul(source_mat, ele_val(source, dim, ele))
      end if
        end do
      end if
      if(ele==100)then
    ...
    !S2023-04~No.005================= MRF Package CounterG !@#$% jxli Apr 2023
    ! add the countergradient correction term to Momentum equation       !!!!!!!!!!!!!!!!!!!!!
    subroutine add_MRF_countergradient_correction_momentum_element_cg(ele, u, du_t, detwei, rhs_addto, counterG_V)
      ...
      tension(3, 1, :) = 0.0
      tension(3, 2, :) = 0.0
      tension(3, 3, :) = counterG_V%val(3, 3, :) ! countergradient correction term
      rhs_addto = rhs_addto - dshape_dot_tensor_rhs(du_t, tension, detwei)

    end subroutine add_MRF_countergradient_correction_momentum_element_cg
    !E2023-04~No.005================= MRF Package CounterG !@#$% jxli Apr 2023
    ...
    subroutine add_viscosity_element_cg_ff(state, ele, test_function, u, oldu_val, nu, x, viscosity, grad_u, &
        fnu, tnu, leonard, strainprod, alpha, gamma, &
    ...
    end subroutine add_viscosity_element_cg_ff
    ...
    subroutine add_viscosity_element_cg(state, ele, test_function, u, oldu_val, nu, x, viscosity, grad_u, &
      ...
      real, dimension(u%dim, u%dim, ele_ngi(u, ele))  :: viscosity_gi_c
      real, dimension(u%dim, ele_loc(u, ele))            ::  nu_ele_c 
      !vis_dim=viscosity%dim
    ...
    ! Assemble P1-P1 stabilisation term in the pressure matrix.
      ...
      INTEGER(C_INTPTR_T) :: ptr_field_val
      INTEGER(C_INTPTR_T) :: ptr_ndglno
      ! transform-to-physical->dshape-tensor-dshape
      INTEGER(C_INTPTR_T) :: ptr_dshape
      INTEGER(C_INTPTR_T) :: ptr_detwei
      ! get-edge-lengths->dshape-tensor-dshape
      INTEGER(C_INTPTR_T) :: ptr_bar
      logical :: gpu_on
    INTEGER :: clock_rate
    real:: start_time, end_time
    real elapsed_time;
   ...
   !     call transform_to_physical(coordinates, ele, p_shape, dshape=loop_dp_t(:,:,:,ele), detwei=loop_detwei(:,ele))
   !   enddo 
    gpu_on=.true.

    start_time = MPI_Wtime()
    call kmk_gpu_alloc(coordinates%dim,ele_ngi(pressure_mesh,1),ele_loc(pressure_mesh,1),&
     ele_count(pressure_mesh),size(coordinates%val,2))
    call alloc_assemble_transfer(kt%val, kt%sparsity%findrm, kt%sparsity%colm, pressure_mesh%ndglno, &
    coordinates%dim, ele_loc(pressure_mesh, 1), ele_ngi(pressure_mesh, 1), ele_count(pressure_mesh), size(kt%sparsity%findrm,1),size(kt%val,1))
    call transform_to_physical_full_GPU(gpu_on, coordinates, p_shape, ele_count(pressure_mesh)) 
    call get_edge_lengths_gpu(coordinates%dim, ele_loc(pressure_mesh, 1), ele_ngi(pressure_mesh, 1), ele_count(pressure_mesh), size(coordinates%val,2),&
     coordinates%val, coordinates%mesh%ndglno)
       
    call assemble_stiff_matrix_gpu(kt%val, kt%sparsity%findrm, kt%sparsity%colm, pressure_mesh%ndglno, &
    coordinates%dim, ele_loc(pressure_mesh, 1), ele_ngi(pressure_mesh, 1), ele_count(pressure_mesh), size(kt%sparsity%findrm,1),size(kt%val,1))
    end_time = MPI_Wtime()
    elapsed_time = end_time - start_time
    write(*,*)"assemble kmk time:",elapsed_time
      call kmk_gpu_dealloc()
      call dealloc_assemble()
      ! by scaling masslump with theta, we divide kmk by theta
      if(abs(theta_pg - 1.0) < epsilon(0.0)) then
        write(*,*)"kmk abs(theta_pg - 1.0)<epsilon(0.0)"
